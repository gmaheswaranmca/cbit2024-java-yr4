...
class Solution {
    ...
    public void dfs(char[][] grid, int r, int c, int m, int n, Set<Cell> visited) {
        ...
        while(!stack.isEmpty()) {
            Cell stackCell = stack.pop();
            //travel in the directions 
            for(Cell cellOffset : directionOffsets) {
                Cell neighbourCell = new Cell(stackCell.row + cellOffset.row, 
                                             stackCell.col + cellOffset.col);
